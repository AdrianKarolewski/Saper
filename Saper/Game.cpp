...
Game::Game(sf::RenderWindow &Saper)
{
    ...
    lv_ea = new Texts(font, "Lv_easy",  static_cast<int>(CHAR_SIZE_1 * window_h),  
        { static_cast < float>(0.156 * window_w),static_cast <float>(0.092 * window_h) }, 
        { static_cast<float>(window_w / 2),static_cast <float>(0.324 * window_h) });
    
    lv_me = new Texts(font, "Lv_medium", static_cast<int>(CHAR_SIZE_1 * window_h), 
        { static_cast <float>(0.156 * window_w),static_cast <float>(0.092 * window_h) }, 
        { static_cast<float>(window_w / 2),static_cast <float>(0.462 * window_h) });
    
    lv_ha = new Texts(font, "Lv_hard", static_cast<int>(CHAR_SIZE_1 * window_h),  
        { static_cast <float>(0.156 * window_w),static_cast <float>(0.092 * window_h) }, 
        { static_cast<float>(window_w / 2),static_cast <float>(0.601 * window_h) });
    
    end_in_menu = new Texts(font, "Exit", static_cast<int>(CHAR_SIZE_1 * window_h),  
        { static_cast <float>(0.156 * window_w),static_cast <float>(0.092 * window_h) }, 
        { static_cast<float>(window_w / 2),static_cast <float>(0.740 * window_h) });

    reset = new Texts(font, "Reset", static_cast<int>(CHAR_SIZE_1 * window_h), 
        { static_cast <float>(0.104 * window_w),static_cast <float>(0.074 * window_h) }, 
        { static_cast<float>(window_w) - static_cast<float>(0.119 * window_w), 
        static_cast <float>(0.120 * window_h) });
    
    back_to_menu = new Texts(font, "Menu", static_cast<int>(CHAR_SIZE_1 * window_h), 
        { static_cast <float>(0.104 * window_w),static_cast <float>(0.074 * window_h) }, 
        { static_cast<float>(window_w) - static_cast<float>(0.119 * window_w), 
        static_cast <float>(0.212 * window_h) });
    
    end_in_game = new Texts(font, "Exit", static_cast<int>(CHAR_SIZE_1 * window_h), 
        { static_cast <float>(0.104 * window_w),static_cast <float>(0.074 * window_h) }, 
        { static_cast<float>(window_w) - static_cast<float>(0.119 * window_w), 
        static_cast <float>(0.305 * window_h) });
        
    win = new sf::RectangleShape({ static_cast <float>(window_w), static_cast <float>(2 * window_h) });
    win->setOrigin({ static_cast <float>(window_w / 2), static_cast <float>(window_h) });
    win->setPosition({ static_cast<float>(window_w) / 2, static_cast<float>(window_h) / 2 });

    lost = new sf::RectangleShape({ static_cast <float>(0.442 * window_w), 
        static_cast <float>(0.694 * window_h) });

    lost->setOrigin({ lost->getSize().x/2, lost->getSize().y/2 });

    lost->setPosition({ static_cast<float>(window_w) / 2, static_cast<float>(window_h) / 2 });

    clock = new Texts(font, "", static_cast<int>(0.037 * window_h),
        { static_cast <float>(0.156 * window_w),static_cast <float>(0.074 * window_h) }, 
        { static_cast <float>(0.104 * window_w),static_cast <float>(0.120 * window_h) });
    n_o_flags = new Texts(font, "", static_cast<int>(0.037 * window_h), 
        { static_cast <float>(0.156 * window_w),static_cast <float>(0.074 * window_h) }, 
        { static_cast <float>(0.104 * window_w),static_cast <float>(0.212 * window_h) });

    lifebuoy = new Texts(font, "Bomb for time", static_cast<int>(0.037 * window_h), 
        { static_cast <float>(0.156 * window_w),static_cast <float>(0.074 * window_h) }, 
        { static_cast <float>(0.104 * window_w),static_cast <float>(0.305 * window_h) });
...
{ 
    ...
    {
        unsigned int mouse_x = event.mouseButton.x;
        unsigned int mouse_y = event.mouseButton.y;
        if (event.mouseButton.button == sf::Mouse::Left)
        {
